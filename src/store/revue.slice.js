...
export const createRevue = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    try {
      ...
      return await RevueService.createRevue(data, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const getAllRevue = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await RevueService.getAllRevue(token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const getRevueById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await RevueService.getRevueById(id, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const deleteRevue = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await RevueService.DeleteRevue(id, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(deleteRevue.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        state.Villess = state.Revues.filter(
